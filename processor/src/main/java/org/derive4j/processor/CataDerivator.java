...
final class CataDerivator implements Derivator {
  ...
  private TypeName cataMapperTypeName(AlgebraicDataType adt, DataConstructor dc) {
    ...
    return adt.dataConstruction().isVisitorDispatch()
        ? argsTypeNames.length == 0
            ...
            : argsTypeNames.length == 1
                ? ParameterizedTypeName.get(
                    ClassName.get(utils.function1Model(adt.deriveConfig().flavour()).samClass()), argsTypeNames[0],
                    TypeName.get(adt.matchMethod().returnTypeVariable()))
                : ParameterizedTypeName.get(
                    adt.deriveConfig().targetClass().className().nestedClass(MapperDerivator.mapperInterfaceName(dc)),
                    concat(
                        concat(dc.typeVariables().stream().map(TypeVariableName::get),
                            fold(mapperDerivator.findInductiveArgument(adt, dc), Stream.of(),
                                tm -> Stream.of(ParameterizedTypeName.get(
        ...
        : TypeName.get(
            utils.types().getDeclaredType(Utils.asTypeElement.visit(dc.deconstructor().visitorType().asElement()).get(),
                dc.deconstructor()
                    .visitorType()
                    .getTypeArguments()
                    .stream()
    ...
    TypeSpec wrapper = TypeSpec.anonymousClassBuilder("")
        ...
        .addMethod(
            MethodSpec.methodBuilder(abstractMethod.getSimpleName().toString())
                ...
                .addStatement("return $L.$L($L)", nameAllocator.get("adt var"),
  ...
  private DeriveResult<DerivedCodeSpec> visitorDispatchImpl(AlgebraicDataType adt, DeclaredType visitorType,
                                ...
                                .joinStringsAsArguments(
                                    constructors.stream()
                                        .map(constructor -> constructor.arguments()
                                            .stream()
                                            .map(DataArguments::getType)
                                            .noneMatch(tm -> utils.types().isSameType(tm,
                                                adt.typeConstructor().declaredType()))
                                                    ? ('\n' + constructor.name())
                                                    : CodeBlock.builder()
                                                        .add("\n($L) -> $L.$L($L)",
                                                            Utils.joinStringsAsArguments(concat(
                                                                ...
                                                                constructor
                                                                    ...
                                                                    .map(fn -> nameAllocator
                                                                        ...
                                                                        .newName(fn, fn + " field")))),
                                                            constructor.name(),
                                                            mapperDerivator.mapperApplyMethod(adt.deriveConfig(),
                                                                constructor),
                                                            Utils.joinStringsAsArguments(concat(
                                                                constructor.arguments()
                                                                    .stream()
                                                                    .map(argument -> utils.types().isSameType(
                                                                        argument.type(),
                                                                        adt.typeConstructor().declaredType())
                                                                            ? ("() -> "
                                                                                + nameAllocator.clone().newName(
    ...
    MethodSpec cataMethod = MethodSpec.methodBuilder("cata")
        ...
        .addParameters(constructors.stream()
            .map(dc -> ParameterSpec.builder(cataMapperTypeName(adt, dc), MapperDerivator.mapperFieldName(dc)).build())
            .collect(toList()))
        .addStatement("$T $L = $L.$L", TypeName.get(visitorType), nameAllocator.get("cata"), wrapper,
