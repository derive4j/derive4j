...
final class DeriveConfigBuilder {
  ...
  DeriveConfigBuilder(DeriveUtils deriveUtils) {
    ...
    TypeElement instanceAnnotation = deriveUtils.elements().getTypeElement(Instances.class.getName());
  ...
  private Function<DeriveConfig, DeriveConfig> deriveConfig(TypeElement typeElement,
    ...
    return of(setInClass, setVisibility, setMake, modInstances).flatMap(Utils::optionalAsStream).reduce(Function::andThen).orElse(identity());
  }
  ...
  private Function<DeriveConfig, DeriveConfig> addToDeriveConfig(TypeElement typeElement,
    ...
    return of(setInClass, setVisibility, modMake, modInstances).flatMap(Utils::optionalAsStream).reduce(Function::andThen).orElse(identity());
  }
  ...
  private ClassName deduceDerivedClassName(String inClassAnnotationValue, TypeElement typeElement) {
