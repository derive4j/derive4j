...
public class OtherwiseMatchingStepDerivator {
  ...
  static TypeSpec otherwiseMatchingStepTypeSpec(AlgebraicDataType adt,
    ...
    return otherwiseMatchBuilder
       ...
       .addMethods(FlavourImpl.findEitherType(deriveContext.flavour(), deriveUtils.elements()).map(eitherType ->
          otherwiseLeftMethod(adt, deriveContext, deriveUtils, eitherType)).orElse(Collections.emptyList()))
       .build();

  }

  private static List<MethodSpec> otherwiseMethods(AlgebraicDataType adt,
     DeriveContext deriveContext, DeriveUtils deriveUtils) {
             ...
             DataConstructions.cases()
                .multipleConstructors(
                   MultipleConstructorsSupport.cases()
                      .visitorDispatch(
                         (visitorParam, visitorType, constructors) -> visitorDispatchImpl(deriveContext, f0, adt, visitorType, visitorParam))
  ...
  private static MethodSpec otherwiseNoneMethod(AlgebraicDataType adt,
     DeriveContext deriveContext, DeriveUtils deriveUtils) {

    FlavourImpl.OptionType optionType = FlavourImpl.findOptionType(deriveContext.flavour(), deriveUtils.elements());

    TypeName returnType = TypeName.get(deriveUtils.types().getDeclaredType(FlavourImpl.findF(deriveContext.flavour(), deriveUtils.elements()),
       adt.typeConstructor().declaredType(), deriveUtils.types().getDeclaredType(optionType.typeElement(), adt.matchMethod().returnTypeVariable())));
  ...
  private static CodeBlock visitorDispatchImpl(DeriveContext deriveContext, TypeElement f0, AlgebraicDataType adt, DeclaredType visitorType,
  ...
  private static CodeBlock functionsDispatchOptionImpl(DeriveUtils deriveUtils, DeriveContext deriveContext, FlavourImpl.OptionType optionType,
    ...
    for (DataConstructor dc : constructors) {
      ...
      String lambdaArgs = joinStringsAsArguments(IntStream.range(8, 8 + dc.arguments().size() + dc.typeRestrictions().size())
         .mapToObj(i -> "$" + i + "L"));
      ...
      codeBlock.addStatement("$1T $2L = (this.$3L != null) ? (" + lambdaArgs + ") -> $4T.$5L(this.$3L.$6L(" + lambdaArgs + "))\n"
         ...
         Stream.concat(
            Stream.of(
               ...
               optionType.someConstructor(),
               getAbstractMethods(dc.deconstructor().visitorType().asElement().getEnclosedElements()).get(0).getSimpleName().toString(),
               optionType.noneConstructor()
            ),
            Stream.concat(dc.arguments().stream(), dc.typeRestrictions().stream().map(TypeRestriction::idFunction))
               .map(DataArgument::fieldName)
               .map(daName -> nameAllocator.get(daName))
  ...
  private static CodeBlock visitorDispatchOptionImpl(DeriveUtils deriveUtils, DeriveContext deriveContext, FlavourImpl.OptionType optionType,
    ...
    CodeBlock lambdaArgs = adt.dataConstruction().constructors().stream().map(dc -> {
      ...
      String lambdaArg = joinStringsAsArguments(IntStream.range(6, 6 + dc.arguments().size() + dc.typeRestrictions().size())
         .mapToObj(i -> "$" + i + "L"));

      return CodeBlock.builder().add("(this.$1L != null) ? (" + lambdaArg + ") -> $2T.$3L(this.$1L.$4L(" + lambdaArg + "))\n"
            + ": (" + lambdaArg + ") -> $2T.$5L()",
         Stream.<Object>concat(Stream.of(
            mapperFieldName(dc),
            ClassName.get(optionType.typeElement()),
            optionType.someConstructor(),
            MapperDerivator.mapperApplyMethod(deriveUtils, deriveContext, dc),
            optionType.noneConstructor()),
            Stream.concat(dc.arguments().stream(), dc.typeRestrictions().stream().map(TypeRestriction::idFunction))
               .map(DataArgument::fieldName)
               .map(daName -> nameAllocator.get(daName))
         ).toArray(Object[]::new)).build();
    }).reduce((cb1, cb2) -> CodeBlock.builder().add(cb1).add(",\n").add(cb2).build()).orElse(CodeBlock.builder().build());
  ...
  private static List<MethodSpec> otherwiseLeftMethod(AlgebraicDataType adt,
     DeriveContext deriveContext, DeriveUtils deriveUtils, FlavourImpl.EitherType eitherType) {
    ...
    TypeName eitherTypeName = deriveUtils.resolveToTypeName(
       eitherTypeElement.asType(), etv ->
          deriveUtils.types().isSameType(etv, eitherTypeElement.getTypeParameters().get(0).asType())
    ...
    return Arrays.asList(MethodSpec.methodBuilder(otherwiseLeftMethodName)
          ...
          .addCode(
             DataConstructions.cases()
                .multipleConstructors(
                   MultipleConstructorsSupport.cases()
                      .visitorDispatch(
                         (visitorParam, visitorType, constructors) ->
                            visitorDispatchEitherImpl(deriveUtils, deriveContext, f0, eitherType, eitherTypeName, adt, visitorType, visitorParam, arg))
                      .functionsDispatch(constructors ->
                         functionsDispatchEitherImpl(deriveUtils, deriveContext, f0, eitherType, eitherTypeName, adt, constructors, arg))
                )
                .otherwise(() -> {
                  throw new IllegalArgumentException();
                })
       ...
       MethodSpec.methodBuilder(otherwiseLeftMethodName)
          .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
          .addTypeVariable(leftTypeVarName)
          .addParameter(leftTypeVarName, arg)
          .addStatement("return this.$L(() -> $L)",
             otherwiseLeftMethodName,
             arg)
          .returns(returnType)
          .build());
  }
  ...
  private static CodeBlock functionsDispatchEitherImpl(DeriveUtils deriveUtils, DeriveContext deriveContext, TypeElement f0,
     FlavourImpl.EitherType eitherType, TypeName eitherTypeName,
    ...
    for (DataConstructor dc : constructors) {
      ...
      codeBlock.addStatement("$1T $2L = (this.$3L != null) ? (" + lambdaArgs + ") -> $4T.$5L(this.$3L.$6L(" + lambdaArgs + "))\n"
         ...
         Stream.concat(
            Stream.of(
               mapperTypeName(adt, dc, deriveContext, deriveUtils, eitherTypeName),
               nameAllocator.get("case var"),
               mapperFieldName(dc),
               ClassName.get(eitherTypeElement),
               getRightConstructor(eitherType),
               getAbstractMethods(dc.deconstructor().visitorType().asElement().getEnclosedElements()).get(0).getSimpleName().toString(),
               getLeftConstructor(eitherType),
               getAbstractMethods(f0.getEnclosedElements()).get(0).getSimpleName().toString())
            ,
            Stream.concat(dc.arguments().stream(), dc.typeRestrictions().stream().map(TypeRestriction::idFunction))
               .map(DataArgument::fieldName)
  ...
  private static CodeBlock visitorDispatchEitherImpl(DeriveUtils deriveUtils, DeriveContext deriveContext, TypeElement f0,
    ...
    CodeBlock lambdaArgs = adt.dataConstruction().constructors().stream().map(dc -> {
      ...
      String lambdaArg = joinStringsAsArguments(IntStream.range(7, 7 + dc.arguments().size() + dc.typeRestrictions().size())
         .mapToObj(i -> "$" + i + "L"));
